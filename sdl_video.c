					...
					if (runtime_options[0].state) {
						...
						if (count == 0) {
							...
							snprintf(text, sizeof text, 
									"UP=%.1s DOWN=%.1s LEFT=%.1s RIGHT=%.1s STR=%.1s",
									keycode_to_keysym(ctrl_remaps[DINGOO_REMAP_UP].remap_id)+5,
									keycode_to_keysym(ctrl_remaps[DINGOO_REMAP_DOWN].remap_id)+5,
									keycode_to_keysym(ctrl_remaps[DINGOO_REMAP_LEFT].remap_id)+5,
									keycode_to_keysym(ctrl_remaps[DINGOO_REMAP_RIGHT].remap_id)+5,
...
//						}else if(count == 2){
//							snprintf(text,sizeof text,"key=%s",keycode_to_keysym(ctrl_remaps[DINGOO_REMAP_UP].remap_id)+5);
							
							
//						}else if(count == 3){

//							snprintf(text,sizeof text,"key=",keycode_to_keysym(ctrl_remaps[DINGOO_REMAP_UP].remap_id)+5);
						}
						#else
						if (count == 0) {
